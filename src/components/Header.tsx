      ...
      <div className="container mx-auto px-4">
        <div className="flex items-center justify-between h-20">
          ...
          <nav className="hidden lg:flex items-center gap-6">
            {navItems.map(item => item.type === "link" ? <Link key={item.id} to={`/${item.id}`} className={`font-medium hover:text-primary transition-smooth uppercase ${isScrolled ? 'text-foreground/80' : 'text-white'}`}>
                  {item.label}
                </Link> : <button key={item.id} onClick={() => handleNavigation(item.id)} className={`font-medium hover:text-primary transition-smooth uppercase ${isScrolled ? 'text-foreground/80' : 'text-white'}`}>
                  {item.label}
                </button>)}
          </nav>
          ...
          {/* Mobile Menu Button */}
          <Button variant="ghost" size="icon" className={`lg:hidden ${!isScrolled && 'text-white hover:text-white'}`} onClick={() => setIsMobileMenuOpen(!isMobileMenuOpen)}>
            {isMobileMenuOpen ? <X className="h-6 w-6" /> : <Menu className="h-6 w-6" />}
          </Button>
        </div>

        {/* Mobile Menu */}
        {isMobileMenuOpen && <div className="lg:hidden pb-4 animate-fade-in">
            <nav className="flex flex-col gap-4">
              {navItems.map(item => item.type === "link" ? <Link key={item.id} to={`/${item.id}`} className="text-left font-medium text-foreground/80 hover:text-primary transition-smooth uppercase" onClick={() => setIsMobileMenuOpen(false)}>
                    {item.label}
                  </Link> : <button key={item.id} onClick={() => handleNavigation(item.id)} className="text-left font-medium text-foreground/80 hover:text-primary transition-smooth uppercase">
                    {item.label}
                  </button>)}
